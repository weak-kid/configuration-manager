...
public:
    ConfigClient()
        : proxy_(sdbus::createProxy(sdbus::ServiceName{SERVICE_NAME}, sdbus::ObjectPath{OBJECT_PATH}))
    {
        proxy_->uponSignal("configurationChanged").onInterface(INTERFACE_NAME).call(
    ...
    void run() {
        worker_ = std::thread([this]() {
            while (!stop_) {
                ...
                if (cv_.wait_for(lock, timeout_, [this] { return stop_.load(); })) break;
...
private:
    void initialLoad() {
        ...
        proxy_->callMethod("GetConfiguration").onInterface(INTERFACE_NAME).storeResultsTo(config);
        updateConfiguration(config);
    }

    void updateConfiguration(const std::map<std::string, sdbus::Variant>& config) {
